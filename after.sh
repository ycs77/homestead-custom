...
if [ ! -f /usr/local/extra_homestead_software_installed ]; then
    ...
    # update php session time
    SESSION_TIME=86400
    for i in {2..4} ; do
        sed -i "s/gc_maxlifetime = 1440/gc_maxlifetime = ${SESSION_TIME}/g" /etc/php/7.${i}/fpm/php.ini
        service php7.${i}-fpm restart
    done

    touch /usr/local/extra_homestead_software_installed
else
    echo 'extra software already installed'
fi
